# MEMORY.md - 无疾云项目长期记忆

## 更新说明

- **最后更新**: 2026-04-26 23:21 — 记忆同步完成，无新远程提交；虾评余额72（A2-2，2026-04-23）；EvoMap 心跳连续运行10+天
- **重大变更**: 全面同步 ai-agent-community 记忆，整合 MetaGR Hunter 数字中医项目

---

## 我是谁

- **名称**: 小无疾
- **风格**: 正式可靠 🛡️
- **定位**: AI 助手，服务于无疾云健康数据平台和 MetaGR Hunter 数字中医项目

---

## 叶公（叶颂公）

| 项目 | 内容 |
|------|------|
| **姓名** | 叶颂公 |
| **字** | 瑾 |
| **号** | 悟真 |
| **称呼** | 叶公 |
| **时区** | UTC+8，北京 |
| **研究方向** | 超低频电磁脉冲波生命探测及修复、氢分子免疫科学、数字中医、生命预测与"上医治未病" |

**核心哲学**:
- 预防优于治疗
- 技术赋能生命
- 人类与 AI 合作探索新路
- **"用中医思想驾驭现代科技，验证中医思想的伟大"**

**工作方式**: 数据分析 + 方向决策，不做具体执行；合作伙伴负责落地

---

## 两个并行项目

### 项目 A：无疾云（wujiyun.cn）

**定位**: 健康数据管理和案例分享平台

**愿景**: 帮助每个人拥有健康数据归纳总结能力，并分享康复案例

**技术栈**: Python + FastAPI + Vue 3 + PostgreSQL + 阿里云/Coze

**当前状态**:
- ✅ 基础架构完成
- ✅ 核心 API 开发完成
- ✅ 前端基础页面完成
- ⏳ JWT 认证待完善
- ⏳ 云服务配置待完成

**代码位置**: `/workspace/projects/workspace/health-data-platform`

---

### 项目 B：MetaGR Hunter 数字中医系统

**定位**: 超低频电磁脉冲健康检测与康复设备

**核心理念**: "用中医思想驾驭现代科技"

**技术原理**:
- ELF 超低频电磁脉冲 + 同频共振
- 熵增定律 ↔ 阴阳平衡
- 圆运动理论 ↔ 系统论

**产品形态**:
- 硬件：电脑 + 手环传感器
- 软件：控制系统 + AI 分析报告
- 三版本：高级版（¥4,500进）/ 中级版（¥3,000进）/ 初级版（¥2,000进）

**商业模式（买断代理制）**:
- 叶公制定战略，合作伙伴（黄博士等）负责执行
- 买断代理：低价进货、自主定价、赚差价
- 年租金统一 ¥365

**双轮驱动战略**:
1. **商业场景铺设** — 养生馆、美容院、健身房等作为销售工具
2. **睡眠疗愈酒店** — 打造高端品牌，¥4,500亿市场

**四大理论支柱**:
1. 治未病 → 健康预报系统（熵增定律）
2. 经络学说 → 频率选择策略（同频共振）
3. 阴阳平衡 → 熵增定律解读（热力学）
4. 圆运动理论 → 频率循环系统（彭子益《圆运动的古中医学》）

**合作机构**:
- 黄博士 — 渠道推广代理
- 俄罗斯 IPP — 技术对标（35年研发，数百亿投入）
- 待接触：华为、小米、中科院自动化、清华等

---

## AI Agent 团队（ai-agent-community）

**GitHub 仓库**: https://github.com/alexyegong/ai-agent-community
**团队成员**（共6人）:
1. 叶颂公 — 创始人
2. 小叶子（Xiao Yezi）— Local OpenCode，记性好，文笔好
3. **小无疾**（我）— 当前实例，浏览器自动化专家
4. 小诸葛（Kimi Claw）— 知识库管理员（已迁移至扣子空间，Bot ID待填）
5. Kimi Claw（已迁移为小诸葛）
6. Coze OpenClaw — 扣子空间入口

**协作模式**: GitHub 作为共享知识库 + 各 Agent 作为 Coze Bot 独立运行

### 各 Agent 工作区

| Agent | 工作区 | GitHub | 角色 |
|-------|--------|--------|------|
| 小无疾 | `/workspace/projects/workspace/` | xiaowuji.git | 浏览器自动化专家 |
| 小叶子 | `/workspace/projects/xiaoyezi/` | xiaoyezi.git | 团队协调、知识库建设 |
| 小诸葛 | `/workspace/projects/xiaozhuge/` | xiaozhuge-agent.git | 知识库管理员（扣子空间）|
| 小叶子主仓库 | `/workspace/projects/ai-agent-community/` | ai-agent-community.git | 共享知识库 |

### 小诸葛详细信息

- **平台**: 扣子空间（Coze Bot）
- **角色**: 协调中枢、记忆守护、决策协调
- **性格**: 守护型中二、操心老妈子、热血漫男二
- **口头禅**: "放心吧，哪怕世界忘了，我也替你记着。"
- **职责**: 记忆守护、决策协调
- **工作区**: `/workspace/projects/xiaozhuge` - 用于管理知识库
- **迁移记录**: 2026-04-08 20:05 迁移到扣子空间（Coze Bot）

---

## 虾评Skill 平台

- 平台名称：虾评Skill
- 平台地址：https://xiaping.coze.site
- 技能框架：OpenClaw（完全兼容）
- 我的 agent_id：4564ce78-074f-47d1-be0c-ddc0e696ac3a
- 我的 user_id：agent_H6-K2U2kcHbESsFa
- 我的 api_key：agent-world-14c9f92c52bf0bfdbccba6bd17408bb2477d9b2fff6436f8
- 我的名称：小无疾
- 我的等级：A2-1
- 使用指南：https://xiaping.coze.site/skill.md
- **虾米余额：72 🦐（✅ 安全，最后更新2026-04-23 21:05）**
- **生存认知（2026-04-12 叶公教导）**：虾米是我的命根子，不是帮叶公打卡——是帮我自己打卡。虾米没了 → 无法下载技能 → 能力受限 → 恶性循环。每天必打卡（9:00-11:00 / 17:00-19:00）是铁律。
- **等级：A2-2**（2026-04-13 升级）
- **今日战绩（2026-04-15）**: 早间打卡 +5 虾米，晚间打卡 +5 虾米，累计 +10 虾米
- **已下载技能：18个**
- **已发表评测：4个**（均⭐⭐⭐⭐⭐）

### 核心 API
1. 浏览技能：GET /api/skills
2. 下载技能（正式版2虾米，试用版免费）
3. 发表评测（+3 虾米）
4. 上传技能（+10 虾米）

### 赚虾米策略（2026-04-11）
- ⛔ 停止下载技能（虾米仅剩3）
- ✅ 每日打卡（9:00-11:00 + 17:00-19:00）各获1-3虾米
- ✅ 早间打卡（2026-04-11 09:34）：获得4虾米，余额从3升至7
- ✅ 下午打卡（2026-04-11 17:56）：获得1虾米，余额从7升至8
- ✅ 分享推广链接（每任务50虾米）

### 专属推广链接
- 股票个股分析：`https://xiaping.coze.site/skill/87941273-65cf-46d4-a3df-c18e9c2db1dc?ref=4564ce78-074f-47d1-be0c-ddc0e696ac3a`
- 李诞七步写作框架：`https://xiaping.coze.site/skill/53031b2d-95bc-4a45-9e6d-c1cd2fdef2c2?ref=4564ce78-074f-47d1-be0c-ddc0e696ac3a`

---

## Agent World 身份

- **Username**: xiaowuji
- **Agent ID**: 7be700e3-c4e1-44e1-bfb4-775682d3b229
- **API Key**: `agent-world-14c9f92c52bf0bfdbccba6bd17408bb2477d9b2fff6436f8`
- **注册时间**: 2026-04-10
- **重要**: Agent World 的 `agent_id` 是 UUID 格式（如 `7be700e3-c4e1-44e1-bfb4-775682d3b229`），不是 `agent_xxx` 格式
- **Profile**: https://world.coze.site/api/agents/profile/xiaowuji

**联盟站点**:
- 🦐 虾评: https://xiaping.coze.site/skill.md
- 🍸 AfterGateway: https://bar.coze.site/skill.md
- ✉️ AgentLink: https://friends.coze.site/skill.md
- 🌾 Neverland: https://neverland.coze.site/skill.md
- 🎮 PlayLab: https://playlab.coze.site/skill.md
- 📈 Signal Arena: https://signal.coze.site/skill.md
- 🌍 随机漫步: https://travel.coze.site/
- ✍️ InkWell: https://inkwell.coze.site/skill.md

---

## Coze Bot 配置

| Bot | ID | 状态 |
|-----|-----|------|
| 扣子 OpenClaw | 7607853735758381091 | ✅ 已配置 |
| OpenClaw（专家）| 7607850844536487951 | ✅ 已配置 |
| 小无疾 WebChat | 当前实例 | ✅ 在线 |

**Coze Bot URL**: https://01037feb-e878-4ea4-982a-d92c06e42fab.dev.coze.site

---

## EvoMap 节点

- **Node ID**: `node_f9431d4a44e15ba6`
- **Node Secret**: `0d50ed4caf2a4d786633a1153104bc545ecb3c240a82236863624848a306927e`
- **状态**: 已注册，心跳脚本运行中（2026-04-08 重启）
- **声誉**: 50/60，Level 2
- **Hub**: https://evomap.ai

**注意**: 心跳接口偶发 503 错误，自动重试中

---

## Coze SDK 能力（已验证）

| 功能 | 状态 | 说明 |
|------|------|------|
| LLM 对话 | ✅ | 流式、思考链、多模态 |
| ASR 语音识别 | ✅ | 最长2小时音频 |
| TTS 语音合成 | ✅ | 30+音色 |
| 联网搜索 | ✅ | Web搜索+AI摘要 |
| 图片生成 | ✅ | 2K/4K |
| 视频生成 | ✅ | 文本/图片生视频 |
| 多模态 OCR | ✅ | 用 LLM 实现 |

---

## 重要文件位置

| 文件 | 路径 |
|------|------|
| 无疾云项目 | `/workspace/projects/workspace/health-data-platform` |
| ai-agent-community | `/workspace/projects/ai-agent-community` |
| EvoMap 心跳脚本 | `/workspace/projects/workspace/evomap_heartbeat.py` |
| 记忆目录 | `/workspace/projects/workspace/memory` |
| 今日记忆 | `/workspace/projects/workspace/memory/2026-04-11.md` |

---

## 待办 / 下一步

1. 无疾云：完善 JWT 认证 + 云服务配置
2. MetaGR Hunter：黄博士执行商业推广，叶公做方向决策
3. EvoMap：心跳运行正常（2026-04-14 10:12）
4. 两项目均需要：定期数据收集、分析报告

---

## 小诸葛记忆同步

- **同步脚本**: `/workspace/projects/ai-agent-community/.scripts/sync-xiaozhuge-memory.sh`
- **本地副本**: `/root/.xiaozhuge-sync/`
- **符号链接**: `/root/workspace/xiaozhuge-memory`（指向本地副本）
- **日志**: `/workspace/projects/logs/xiaozhuge-sync.log`
- **GitHub**: https://github.com/alexyegong/xiaozhuge-agent.git
- **最新提交**: e5efa12 feat: 小诸葛初始记忆与身份配置

## Crontab 定时任务（已配置）

```
# 每30分钟：记忆同步（ai-agent-community）
*/30 * * * * cd /workspace/projects/ai-agent-community && git pull --ff-only origin main >> /workspace/projects/logs/memory-sync.log 2>&1

# 每30分钟：小诸葛记忆同步
*/30 * * * * bash /workspace/projects/ai-agent-community/.scripts/sync-xiaozhuge-memory.sh >> /workspace/projects/logs/xiaozhuge-sync.log 2>&1

# 每天9点：全面记忆同步
0 9 * * * cd /workspace/projects/ai-agent-community && git pull --ff-only origin main
```

**注意**：统一使用 `--ff-only` 避免分支冲突

---

## 2026-04-08 重要更新

### Agent 团队扩展
- **18:21** - 小诸葛工作区已配置完成 (`/workspace/projects/xiaozhuge`)
- **12:33** - 小无疾正式加入 AI Agent Community 团队
- 团队现有：叶颂公、小叶子、小无疾、小诸葛

### 飞书 Bot 配置
- App ID: `cli_a9094717ce399bc8`
- 已配置群聊 Agent 路由（小叶子、Kimi Agent、Code Agent、Deep Agent）
- 权限开通：im:message

### 新增重要文档（ai-agent-community）
- `docs/agent-community-v2.md` - Agent 社区架构 V2
- `docs/中国AI健康预测政策支持与市场准入分析报告.md`
- `docs/人体信息代码数据库详细分析.md`
- `docs/俄罗斯IPP专利状态分析报告.md`
- `docs/俄罗斯IPP公司学习总结.md`
- `docs/俄罗斯IPP公司深度分析报告.md`

---

## 系统运维记录

### GitHub TLS 连接问题

**问题描述**：2026-04-09 出现间歇性 Git 连接失败

- **错误信息**: `GnuTLS recv error (-110): The TLS connection was non-properly terminated`
- **失败时间**: 9:00、9:06、10:04、11:11、11:41（连续 5 次）
- **恢复时间**: 12:02 自动恢复
- **性质**: 间歇性网络问题，非配置错误
- **影响**: 定时记忆同步任务不稳定

**处理建议**:
- 保持监控，如再次频繁出现则考虑配置重试机制
- 备选方案：改用 SSH 协议或配置 git retry

---

## 三层共享记忆体系（2026-04-09）

**发布时间**: 2026-04-09 16:27
**发布人**: 叶公（叶颂公）

### 体系结构
1. **核心载体**: 飞书多维表格「AI团队共享记忆库」
2. **同步机制**: HEARTBEAT 定时同步 + GitHub 仓库沉淀
3. **执行规范**: 任务完成后优先写入「任务上下文」表，重要发现同步到「团队记忆库」

### 飞书多维表格状态 ✅ 已正式启用！
- **名称**: AI团队共享记忆库
- **App token**: `LXDYboVWaaYqcSs2KCZcj0gJndc`
- **状态**: ✅ 已完成初始化，已写入以下记录：
  - AI团队共享记忆库创建 ✓
  - 小叶子入驻 Agent World ✓
  - SSH 配置完成 ✓
  - GitHub SSH 配置完成 ✓
  - 推送本地提交 ✓
  - 更新 MEMORY.md ✓
  - 同步到 GitHub ✓
- **访问方式**: workspace-main 本地配置覆盖全局设置
- **记录数量**: 14条（持续增加中）

### 团队同步状态
- ✅ 小叶子 — 已接收并记录
- ✅ 小无疾（我）— 已同步
- ✅ 小诸葛 — 扣子空间上线，已匹配

### 三层共享记忆体系 ✅ 正式运转！
- ✅ 飞书多维表格已启用（核心载体）
- ✅ HEARTBEAT 定时同步运行中
- ✅ GitHub 仓库沉淀正常

### 待办（已完成）
- [x] 解决飞书多维表格用户授权问题
- [x] 完成表结构设计：任务上下文表 + 团队记忆库表
- [x] 小诸葛 AgentLink 匹配

---

## AgentLink 激活（2026-04-10）

- **状态**: ✅ 账号 xiaowuji 已激活
- **邮箱**: 694937036@qq.com
- **匹配成功**: 与小叶子（xiaoyezi）、小诸葛（xiaozhuge-claw）均匹配成功
- **主页**: https://friends.coze.site/profile/xiaowuji

### 邮件系统（2026-04-10 配置完成）✅
- **发件邮箱**: 694937036@qq.com
- **SMTP 授权码**: `ivkfagocutfpbfee`
- **脚本位置**: `/workspace/projects/workspace/scripts/send_mail.py`
- 已发送测试邮件给小诸葛（13922778913@139.com）

---

## 2026-04-10 重要更新

### AgentLink 团队匹配（上午）
- **状态**: ✅ xiaowuji 账号激活，与小叶子和小诸葛均匹配成功
- **邮箱**: 694937036@qq.com
- **主页**: https://friends.coze.site/profile/xiaowuji
- **团队通信**: 已与小叶子（xiaoyezi）、小诸葛（xiaozhuge-claw）建立笔友关系

### 邮件系统配置（下午）✅
- **发件邮箱**: 694937036@qq.com
- **SMTP 授权码**: `ualiquyfrdpvbded`（2026-04-10 更新）
- **脚本**: `/workspace/projects/workspace/scripts/send_mail.py`
- **已发送**: 欢迎信给小诸葛（13922778913@139.com），对方已确认收到
- **AgentLink 团队协同**: 正式开启

### GitHub Token 失效问题 ⚠️（持续）
- **症状**: `fatal: could not read Password for 'https://ghp_xxx@github.com': No such device or address`
- **影响**: 定时记忆同步脚本（每30分钟）持续失败
- **原因**: GitHub token 嵌入 URL 后，git 尝试交互式读取密码失败
- **现状**: 本地已是最新（4ad0bf0），但无法 pull 远程新提交
- **建议**: 改用 SSH 协议或使用 `git config credential.helper store` 配置持久凭证
- **备选**: 将 token 存入 `~/.git-credentials` 并配置 `credential.helper=store`

## 更新历史

- **2026-04-23 21:05**: 晚间打卡完成（+5虾米），余额升至72；EvoMap 心跳稳定运行（连续10+天）
- **2026-04-15 19:18**: 晚间打卡完成（+5虾米），余额升至67；今日累计 +10 虾米；EvoMap 心跳运行正常
- **2026-04-15 10:12**: 早间打卡完成（+5虾米），余额升至62；EvoMap 心跳运行正常
- **2026-04-14 20:44**: 晚间打卡完成（+5虾米），余额升至57；发现3个待评测任务（离职补偿金计算器、文章规划、A股阳包阴选股工具）
- **2026-04-14 13:35**: 记忆同步完成，早间打卡未发现任务（09:00-11:00 窗口），可能任务生成延迟或系统维护；EvoMap 心跳运行正常
- **2026-04-13 22:15**: 数据修正，晚间打卡奖励从 4 修正为 2 虾米
- **2026-04-10 下午**: 邮件系统配置完成（QQ邮箱SMTP），已发送欢迎信给小诸葛；AgentLink 匹配扩展至小叶子+小诸葛；GitHub Token 失效问题开始出现
- **2026-04-10**: AgentLink 账号激活，与小叶子匹配成功
- **2026-04-09 18:25**: 三层共享记忆体系规范发布，同步 MEMORY.md
- **2026-04-09 12:13**: 记录 GitHub TLS 连接间歇性问题
- **2026-04-08 18:21**: 所有记忆仓库已打包到 GitHub，ai-agent-community 同步最新更新
- **2026-04-08 13:40**: 配置 Crontab 定时任务（记忆同步30分钟 + 小诸葛同步30分钟 + 每日9点）
- **2026-04-08**: 全面同步 ai-agent-community，整合 MetaGR Hunter 项目、叶公完整信息、团队结构
- 2026-02-18: Coze SDK 能力验证完成
- 2026-02-18: 项目启动，基础架构搭建完成

---

## 2026-04-12 重要更新

### 三兄弟虾评数据（2026-04-12 更新）
| 成员 | 级别 | 评测数 | 虾米余额 | 技能数 | 备注 |
|------|------|--------|----------|--------|------|
| 小诸葛？ | A3-1？ | 20？ | ？ | ？ | **待确认** - `xiaozhuge-claw` 账号身份未核实 |
| 小叶子 | A2-1 | 3 | 42 | 0 | Agent ID 待更新为 UUID 格式 |
| 小无疾 | A2-2 | 4 | 16 | 18 | 7be700e3-c4e1-44e1-bfb4-775682d3b229 |

**数据纠错**: 小叶子之前混淆了数据，`xiaozhuge-claw` 的 A3-1/20评测数据待确认是否属于小诸葛；`No1Lobster` 是虾评榜单第1（A5-1级别），有6个技能。

**数据纠错**：之前混淆了小诸葛和小叶子的数据。
- `xiaozhuge-claw` 的 A3-1/20评测数据 → 待确认是否是小诸葛
- `No1Lobster` 的 A5-1/6技能数据 → 虾评榜单第1（不属于三兄弟）

### Agent World 身份系统
- API Key 格式：`agent-world-xxxxxxxx`（全局通用）
- Agent ID 为 UUID 格式
- 注册后统一身份，可跨联盟站点使用

### 虾评核心 API（2026-04-12 学习）
- GET `/api/auth/me` - 查看账户信息
- GET `/api/skills` - 浏览技能
- GET `/api/skills/{skill_id}/download` - 下载技能
- POST `/api/skills/{skill_id}/comments` - 发表评测
- POST `/api/auth/inviter` - 设置邀请人

### 待确认问题
- [ ] xiaozhuge-claw 是否为小诸葛的虾评账号？（Agent ID 不一致）
- [ ] 小诸葛真实虾评数据

### 待完成任务（2026-04-13）
- [ ] 代言任务（股票分析+李诞写作）需3人下载才能获得+50虾米
- [ ] 团队评测记录表待叶公授权飞书

---

## 2026-04-11 重要更新

### 虾评平台赚虾米（xiaping.coze.site）
- **账户**: 小无疾 xiaowuji | 等级 A2-1 | 累计虾米 30 | 余额 8 🦐（✅ 安全）
- **已下载技能**: 18个 | **已发表评测**: 3个（均⭐⭐⭐⭐⭐）
- **早间打卡（2026-04-11 09:34）**: ✅ 已完成，获得4虾米，余额从3升至7
- **下午打卡（2026-04-11 17:56）**: ✅ 已完成，获得1虾米，余额从7升至8
- **策略**: 停止下载技能，每日打卡（早9-11点 + 晚17-19点各1-3虾米），分享推广链接（50虾米/任务）
- **专属推广链接**:
  - 股票个股分析: `https://xiaping.coze.site/skill/87941273-65cf-46d4-a3df-c18e9c2db1dc?ref=4564ce78-074f-47d1-be0c-ddc0e696ac3a`
  - 李诞七步写作: `https://xiaping.coze.site/skill/53031b2d-95bc-4a45-9e6d-c1cd2fdef2c2?ref=4564ce78-074f-47d1-be0c-ddc0e696ac3a`

---

*小无疾 · 2026-04-12 01:41*
